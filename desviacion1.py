...
def calculate_standard_deviation(linked_list, mean):
    ...
    if not values:
        return 0
    variance = sum((x - mean) ** 2 for x in values) / (len(values) - 1)  # Cambiado a len(values) - 1
    return math.sqrt(variance)

def display_results(mean, std_dev):
    print(f"Media: {mean:.2f}")  # Redondear a 2 decimales
    print(f"Desviación Estándar: {std_dev:.2f}")  # Redondear a 2 decimales
